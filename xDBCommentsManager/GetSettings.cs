...
namespace xDBCommentsManager
{
    ...
    {
        public CommonAttributes GetCommonSetting(Database db = null)
        {
            Database context;
            if (db != null)
            {
            ...
            {
                commonAttributes.PageSize = int.Parse(commonSettingItem.Fields["Comment Editor Page Size"].ToString());
                commonAttributes.CommentOnPageLoad = int.Parse(commonSettingItem.Fields["Comments On Page Load"].ToString());
                commonAttributes.CommentOnLoadMore = int.Parse(commonSettingItem.Fields["Comments On Load More Button Click"].ToString());          
            }
            else
            {
                commonAttributes.PageSize = int.Parse(Settings.GetSetting("COMMENT_EDITOR_PAGE_SIZE"));
                commonAttributes.CommentOnPageLoad = int.Parse(Settings.GetSetting("COMMENTS_ON_PAGE_LOAD"));
                commonAttributes.CommentOnLoadMore = int.Parse(Settings.GetSetting("COMMENTS_ON_LOAD_MORE_CLICK"));               
            }
            commonAttributes.Enable = Settings.GetSetting("ENABLE_REPLY");
            commonAttributes.Level = int.Parse(Settings.GetSetting("REPLY_LEVEL"));
